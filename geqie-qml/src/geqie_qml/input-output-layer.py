...
class GradientFunction_for_CNN_feature_maps(torch.autograd.Function):
	...
	@staticmethod
	def forward(ctx,
			 feature_maps_for_numpy: np.ndarray,
			 ...
			 encoding_params: dict = None) -> torch.Tensor:
		"""
		...
		"""
		batch_size, feature_maps, height, width = feature_maps_for_numpy.shape # (batch_size, feature_maps, height, width)
		...
		quantum_circuit_outputs_probabilities = np.empty((batch_size, feature_maps, 2**output_qubits), dtype=np.float32) # (batch_size, feature_maps_size, 2**output_qubits)

		for j in range(batch_size): # iterate over the batch dimension
			for k in range(feature_maps): # iterate over the feature maps dimension
				feature_map_image = feature_maps_for_numpy[j, k] # we treat feature maps as images
				feature_map_image_np = np.array(feature_map_image, dtype=np.float32)
				...
				qc.append(UnitaryGate(geqie_circuit), range(num_qubits))
				...
				output_probs = qnn.forward(input_data=None, weights=vqc_weights[k]).flatten() # (2**output_qubits,)
		...
		return torch.tensor(quantum_circuit_outputs_probabilities, dtype=torch.float32) # (batch_size, feature_maps, 2**output_qubits)
	...
	def backward(ctx, upstream_gradient: torch.Tensor):
		...
		batch_size, feature_maps, height, width = feature_maps_for_numpy.shape
		quantum_circuit_gradients = np.empty((batch_size, feature_maps, 2**output_qubits, num_params), dtype=np.float32) # (batch_size, feature_maps, 2**output_qubits, num_params)
		...
		for j in range(batch_size): # iterate over the batch dimension
			for k in range(feature_maps): # iterate over the feature maps dimension
				...
				_, single_grad = qnn.backward(input_data=None, weights=vqc_weights_np[k])
				quantum_circuit_gradients[j, k] = single_grad

		"""
		2. Placeholder for evaluating dL/dtheta = dL/dp * dp/dtheta
		"""


		"""
		3. Placeholder for evaluating following:
		 3.1 dp/dx
...
class QuantumLayer_for_CNN_feature_maps(nn.Module):
	"""
	...
	Recomended model for (16, 16):
		...
		model = nn.Sequential(
			cnn_layer,
			QuantumLayer_for_CNN_feature_maps(num_qubits=9, num_layers=1),
	...
	def __init__(
		...
		self.ansatz_factory: callable = ansatz_factory
	...
	def forward(self, batched_feature_maps_original: torch.Tensor, **kwargs) -> torch.Tensor:
		...
		# 1. Extract the tensor from the input, and convert it to numpy array (to fit into the GEQIE). Original CNN feature maps: (batch_size, feature_maps_size, height, width)
		feature_maps_for_numpy = (
		...
		quantum_output = GradientFunction_for_CNN_feature_maps.apply(
			feature_maps_for_numpy=feature_maps_for_numpy,
			geqie_encoding=geqie_encoding,
