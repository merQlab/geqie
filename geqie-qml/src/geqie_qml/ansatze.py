...
def build_real_amplitudes_then_adaptive_qcnn(input_qubits: int, output_qubits: int, num_layers: int = 1, **_):
	"""RealAmplitudes on all qubits, followed by QCNN (convolution + pooling) compression.

	Opposite ordering to ``build_adaptive_qcnn_with_real_amplitudes``: a ``RealAmplitudes`` circuit
	with ``num_layers`` repetitions is applied to *all* ``input_qubits`` qubits first, and the QCNN
	then reduces those ``input_qubits`` down to ``output_qubits`` meaningful qubits, exactly as in
	...
	# ================================================================================================
	# 0. REAL AMPLITUDES ON ALL QUBITS (applied before the QCNN compression):
	# ================================================================================================
	ra_layer = build_real_amplitudes_layer(input_qubits, num_layers, next_thetas, label="RealAmplitudes")
	qcnn_circuit.append(
		ra_layer.to_instruction(label="RealAmp"),
