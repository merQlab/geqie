...
class ParameterShiftFunction(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, states: torch.Tensor, weights: torch.Tensor, ops: list[dict], n_qubits: int) -> torch.Tensor:
    ...
    def backward(ctx, grad_output: torch.Tensor) -> tuple[None, torch.Tensor, None, None]:
        ...
        gradient_np = grad_output.detach().cpu().numpy().astype(np.float64)
        ...
        for k, (gate_i, param_idx) in enumerate(param_positions):
            ...
            s_plus  = apply_1q(prefix, param_1q(theta + shift, op["generator"]), op["qubits"][0], n_qubits)
            s_minus = apply_1q(prefix, param_1q(theta - shift, op["generator"]), op["qubits"][0], n_qubits)

            # Run only the suffix — gates after gate_i — with original weights
            s_plus  = run_ops(ops, s_plus,  weights_np, n_qubits, start=gate_i + 1)
            s_minus = run_ops(ops, s_minus, weights_np, n_qubits, start=gate_i + 1)
...
def parse_circuit(ansatz: QuantumCircuit) -> list[dict]:
    ...
    * ``'1q_param'`` — single-qubit Pauli-axis rotation with a symbolic parameter:
      keys ``generator`` (2x2 generator ndarray), ``qubits`` (list[int]),
      ``param_idx`` (int)
    * ``'1q_fixed'`` — single-qubit gate with a fixed matrix:
      keys ``mat`` (2x2 ndarray), ``qubits`` (list[int])
    * ``'2q_fixed'`` — any fixed two-qubit gate (CX, CZ, SWAP, ...):
      keys ``mat`` (4x4 ndarray), ``qubits`` (list[int])
    ...
    for instruction in ansatz.data:
        ...
        if gate.num_qubits == 1 and symbolic:
            ...
            ops.append({
                "type": "1q_param",
                "generator": extract_1q_pauli_generator(gate, p0),
                "qubits": qubits,
...
def apply_op(psi: np.ndarray, op: dict, weights: np.ndarray, n: int) -> np.ndarray:
    ...
    t = op["type"]
    if t == "1q_param":
        return apply_1q(psi, param_1q(float(weights[op["param_idx"]]), op["generator"]),
                        op["qubits"][0], n)
    elif t == "1q_fixed":
...
def run_ops(ops: list[dict], psi: np.ndarray, weights: np.ndarray, n: int, start: int = 0) -> np.ndarray:
...
def apply_1q(psi: np.ndarray, matrix: np.ndarray, qubit_index: int, n: int) -> np.ndarray:
    """
    Apply a 2x2 single-qubit gate to one qubit of a batch of statevectors.

    Parameters
    ----------
    psi          : (batch, dim) complex ndarray
    matrix       : (2, 2) complex ndarray
    qubit_index  : target qubit index (0 = LSB in Qiskit convention)
    ...
    """
    batch, dim = psi.shape
    axis = n - qubit_index                          # qubit axis in (batch, 2, ..., 2) tensor
    psi = psi.reshape(batch, *([2] * n))
    psi = np.tensordot(matrix, psi, axes=([1], [axis]))
    psi = np.moveaxis(psi, 0, axis)
    return psi.reshape(batch, dim)


def apply_2q(psi: np.ndarray, matrix: np.ndarray, qubits: list[int], n: int) -> np.ndarray:
    """
    Apply a 4x4 two-qubit gate to a batch of statevectors.

    ``matrix`` must be in Qiskit's little-endian 2-qubit convention:
    ``matrix[2*b1+b0, 2*c1+c0]`` where b0/c0 = first-arg bit, b1/c1 = second-arg
    bit — this matches ``gate.to_matrix()`` for standard gates (CX, CZ, SWAP).

    Parameters
    ----------
    psi    : (batch, dim) complex ndarray
    matrix : (4, 4) complex ndarray
    ...
    axis_0 = n - q0                # qubit q0 axis in (batch, 2,...,2) tensor
    axis_1 = n - q1                # qubit q1 axis
    matrix4 = matrix.reshape(2, 2, 2, 2)      # (out_q1, out_q0, in_q1, in_q0)
    psi = psi.reshape(batch, *([2] * n))
    psi = np.tensordot(matrix4, psi, axes=([2, 3], [axis_1, axis_0]))
    psi = np.moveaxis(psi, [0, 1], [axis_1, axis_0])
    return psi.reshape(batch, dim)
...
def param_1q(theta: float, pauli_generator: np.ndarray) -> np.ndarray:
...
def extract_1q_pauli_generator(gate: Gate, parameter: Parameter) -> np.ndarray:
    ...
    The extraction samples the gate at a single angle (``θ = π``) and therefore
    assumes the angle is the *bare* parameter ``θ``.  A consistency check at a
    second, non-trivial angle rejects any gate whose angle is a non-identity
    expression of the parameter (e.g. ``rz(2·θ)`` or ``rx(θ + c)``), for which
    the reconstructed ``exp(-i θ P/2)`` and the π/2 parameter-shift rule would
    silently be wrong.
    ...
    ValueError
        ...
        invalid.
    NotImplementedError
        If the gate's angle is not the bare parameter ``θ`` (i.e. it involves a
    ...
    pauli_generator = 1j * U_pi

    if not np.allclose(pauli_generator, pauli_generator.conj().T, atol=1e-7):
        raise ValueError(
            f"Parameterized gate '{gate.name}' has a non-Hermitian generator; "
            "only Pauli-axis rotations (rx/ry/rz/...) can be parameterized."
        )
    if not np.allclose(pauli_generator @ pauli_generator, I2, atol=1e-7):
        raise ValueError(
            f"Parameterized gate '{gate.name}' generator is not involutory "
            "(P² ≠ I); the π/2 parameter-shift rule does not apply."
        )

    # Verify the angle is the bare parameter θ (not a scaling/offset of it) by
    # comparing the reconstructed exp(-i θ P/2) against the actual gate matrix
    # at a second, non-special angle.  A mismatch means the extraction at θ=π
    # does not describe the gate everywhere, so the shift rule would be invalid.
    theta_test = 0.7
    bound_test = circuit.assign_parameters({parameter: theta_test})
    u_test = np.asarray(bound_test.data[0].operation.to_matrix(), dtype=np.complex128)
    if not np.allclose(u_test, param_1q(theta_test, pauli_generator), atol=1e-7):
        raise NotImplementedError(
            f"Parameterized gate '{gate.name}' angle is not the bare parameter "
            "θ (it appears to involve a scaling and/or offset). Only gates of "
            "the form exp(-i θ P/2) with θ as the free parameter are supported."
        )
    return pauli_generator
